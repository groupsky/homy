...
export function detectChangedServices(
  ...
  // Multiple services can share the same build context, so use string[]
  ...
  for (const service of services) {
    ...
    if (directory && VALID_DIR_PATTERN.test(directory)) {
      if (!dirToServices.has(directory)) {
        dirToServices.set(directory, []);
      }
      dirToServices.get(directory)!.push(service.service_name);
    }
  }
  ...
  for (const line of output.split('\n')) {
    ...
    if (parts.length >= 2 && parts[0] === 'docker') {
      ...
      // Look up all service names sharing this directory
      const serviceNames = dirToServices.get(directory);
      if (serviceNames) {
        // Add all services sharing this build context
