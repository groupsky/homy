...
#### `bath-lights` - Occupancy-based lighting with command verification
...
- **Command Verification**: Optional state-based verification with retry logic
- **Configuration**:
  ```javascript
  {
    type: 'bath-lights',
    ...
    commandConfig: {verification: 5000, maxRetries: 3, retryDelay: 1000} // Optional
  }
  ```
...
### Command Verification Implementation

When implementing command verification in bots:

1. **Make it optional**: Always include default configuration for backward compatibility
2. **State-based tracking**: Use device state feedback rather than command IDs
3. **Failure events**: Publish standardized failure events for monitoring
4. **Manual override handling**: Gracefully handle external state changes

Example implementation:
```javascript
const smartPublish = (topic, payload, reason = 'unknown') => {
    if (config.verification === 0 || config.maxRetries === 0) {
        // Legacy mode - direct publish
        mqtt.publish(topic, payload)
        return
    }
    
    // Verification mode with retry logic
...
- Test both legacy mode and enhanced features (like command verification)
...
### Failure Events
When bots encounter errors, they should publish standardized failure events:
```javascript
{
    _type: 'command-verification',
    type: 'command_failed',
    ...
    actualState: actualDeviceState,
    timestamp: Date.now()
}
```

These events are automatically collected by the `mqtt-influx-automation` service and visualized in Grafana dashboards.
