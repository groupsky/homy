...
- **mqtt-influx-automation**: Automation events and command verification (`homy/automation/+/command_failed`)
...
- **command-verification**: Bath-lights command verification events

### Command Verification Converter
Handles automation failure events:
...
- `homy/automation/{controller}/command_failed` - Command verification failure events
...
SELECT mean(100 - "failure_rate_percent") 
FROM "verification_summary" 
WHERE $timeFilter 
GROUP BY time($__interval), "controller"

-- Recent failures
SELECT time, "controller", "reason", "attempts" 
