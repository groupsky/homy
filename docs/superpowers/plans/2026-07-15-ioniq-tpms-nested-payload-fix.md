...
- **`persistedCache.version` stays `1`.** The internal `raw` fingerprint keeps its flat key names (`'fl.psi'`), so no migration is needed. (Prod's stored `lastRaw` is all-`undefined` values, which `fast-json-stable-stringify` serializes to `{}` — any first real reading differs and emits.)
